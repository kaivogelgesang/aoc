...
const PRIMES: [i64; 9] = [2, 3, 5, 7, 11, 13, 17, 19, 23];
...
fn main() {
    ...
    let mut items = input
        .iter()
        .map(|Monkey { items, .. }| items.iter().map(|i| Cheese::from(*i)).collect::<Vec<_>>())
        .collect::<Vec<_>>();
    ...
    for round in 0..10_000 {
        ...
        ) in input.iter().enumerate()
        {
            ...
            for item in current {
                ...
                let target = if a[prime_index] == 0 { t } else { f };
