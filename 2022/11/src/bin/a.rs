...
fn main() {
    let input = parse();
    let mut items = input.iter().map(|Monkey { items, .. }| items.clone()).collect::<Vec<_>>();
    ...
    for _ in 0..20 {
        for (i, Monkey { operation: (l, op, r), test: (m, t, f), .. }) in input.iter().enumerate() {
            ...
            for item in current {
                let l = match l { Operand::Old => item, Operand::Num(n) => *n };
                let r = match r { Operand::Old => item, Operand::Num(n) => *n };
                let item = match op {
                    Operation::Add => l + r,
                    Operation::Sub => l - r,
                    Operation::Mul => l * r,
